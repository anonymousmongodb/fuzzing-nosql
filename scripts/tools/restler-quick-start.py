...
def replay_bug(ip, port, host, use_ssl, restler_dll_path, replay_log, result_dir):
...
def replay_from_dir(ip, port, host, use_ssl, restler_dll_path, replay_dir, result_dir):
    import glob
    from pathlib import Path
    ...
    for file_path in bug_buckets:
        ...
        replay_bug(ip, port, host, use_ssl, restler_dll_path, Path(file_path).absolute(), result_dir)
    pass

def test_spec(ip, port, host, use_ssl, restler_dll_path, task, result_dir):
...
if __name__ == '__main__':
    ...
    if args.task == "replay":
        replay_from_dir(args.ip, args.port, args.host, args.use_ssl, restler_dll_path.absolute(), args.replay_bug_buckets_dir,args.result_dir)
    else:
        ...
        test_spec(args.ip, args.port, args.host, args.use_ssl, restler_dll_path.absolute(), args.task, args.result_dir)
